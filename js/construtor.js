// js/construtor.js - Motor Completo: Cutaway The Sims, Telhados Dinâmicos e Anti-Crash
...
export let modoVisaoAtual = 'cut'; // The Sims Padrão: Paredes Cortadas
...
function paredeQueBloqueia(x1, z1, x2, z2) { 
    return paredesConstruidas.find(p => p.nivel === configsCamera.nivel && !p.isCerca && doLineSegmentsIntersect(x1, z1, x2, z2, p.ax, p.az, p.bx, p.bz)) || null; 
}
...
function finalizarPintura(mesh) { 
    ...
    if (p) {
        p.newMats = Array.isArray(mesh.material) ? mesh.material.map(m => m ? m.clone() : materialParede.clone()) : mesh.material.clone();
    }
}
...
// 🔥 ARMADURA ANTI-CRASH DO RAYCASTER 🔥
function raycastObjetosDoNivel(clientX, clientY) { 
    mouseNdc.x = (clientX / window.innerWidth) * 2 - 1; mouseNdc.y = -(clientY / window.innerHeight) * 2 + 1; 
    raycaster.setFromCamera(mouseNdc, camera); 
    
    const objetosNivel = [];
    paredesConstruidas.forEach(p => { if (p.nivel === configsCamera.nivel && p.mesh) objetosNivel.push(p.mesh); });
    pilaresConstruidos.forEach(p => { if (p.nivel === configsCamera.nivel && p.mesh) objetosNivel.push(p.mesh); });
    pisosConstruidos.forEach(p => { if (p.nivel === configsCamera.nivel && p.mesh) objetosNivel.push(p.mesh); });
    colunasSustentacao.forEach(c => { if (c.nivel === configsCamera.nivel && c.mesh) objetosNivel.push(c.mesh); });
    escadasConstruidas.forEach(e => { if (e.nivel === configsCamera.nivel && e.mesh) objetosNivel.push(e.mesh); });
...
// 🔥 A NOVA MATEMÁTICA QUE GARANTE QUE O TELHADO COMBINE COM A SALA 🔥
function verificarSalasFechadas(px, pz, pontoA) {
    ...
    for (const pt of pontosTeste) {
        ...
        if (fechada && celulas.length > 0) {
            ...
            if (!existe) {
                // Passa o formato da ferramenta para que a reconstrução molde as águas
                criarTelhado(minX, minZ, maxX, maxZ, 3.0, configsCamera.nivel, modoAtivo);
                showAviso("🏠 Cômodo fechado detectado! Telhado gerado.");
                telhadoGerado = true;
            }
        }
    }
...
function criarTelhado(ax, az, bx, bz, alturaTelhado = 3.0, targetNivel = configsCamera.nivel, formato = 'retangulo') { 
    ...
    const telhado = { id, mesh: telhadoMesh, ax, az, bx, bz, alturaTelhado, largura, profundidade, nivel: targetNivel, textura: null, isTelhado: true, formato }; 
...
export function reconstruirTelhadoPiramide(telhado) {
    ...
    let segments = 4;
    if (telhado.formato === 'triangulo') segments = 3;
    if (telhado.formato === 'octogono') segments = 8;

    const geo = new THREE.ConeGeometry(Math.SQRT2 / 2, 1, segments); 
...
function executarMarreta(hitObject) { 
  ...
  if (parede) { 
      ...
      telhadosAfetados.forEach(t => {
          ...
          const { fechada } = encontrarAreaFechada(snapCentroCelula(cx), snapCentroCelula(cz));
          if (!fechada) removerObjetoMundo('telhado', t, telhadosConstruidos);
      });
      return; 
  } 
...
canvas?.addEventListener('pointerdown', e => {
  ...
  if (isRemocao && modoAtivo !== 'pintura') { const hit = raycastObjetosDoNivel(e.clientX, e.clientY); if (hit) executarMarreta(hit.object); return; }
  ...
  if (modoAtivo === 'pintura') {
      ...
      if (e.shiftKey && !isTelhado) {
          ...
          const { celulas, fechada } = encontrarAreaFechada(startX, startZ);
          if (!fechada && !isRemocao) { showAviso("⚠️ O cômodo está aberto! Feche as paredes para usar o Preenchimento (Shift)."); return; }
...
function aplicarMaterialNaFace(mesh, faceIndex, item) { 
    ...
    if (Array.isArray(mesh.material)) {
        ...
        for (let i = 0; i < groupsLen; i++) novosMateriais.push(mesh.material ? mesh.material.clone() : materialParede.clone());
    }

    novosMateriais[safeFaceIndex] = gerarMaterialPintura(item, repeatX, repeatY); 
    mesh.material = novosMateriais; finalizarPintura(mesh); 
}
...
canvas?.addEventListener('pointermove', e => {
  ...
  const isErasePreview = e.ctrlKey || e.metaKey || e.buttons === 2;
  ...
  if (hit) {
    ...
    if (modoAtivo === 'coluna' && !isErasePreview) {
...
// 🔥 NOVO CUTAWAY - VISÃO DE PAREDE "THE SIMS" 🔥
export function atualizarVisibilidadeAndares(modoVisaoManual) {
  ...
  const camDir = new THREE.Vector3().subVectors(orbitAlvo, camera.position);
  ...
  const aplicarParede = (obj) => { 
      if (obj.nivel > configsCamera.nivel) { 
          ...
          if (modoVisaoAtual === 'full') { 
              ...
              obj.mesh.scale.y = 0.15; obj.mesh.position.y = (obj.nivel * obj.altura) + (obj.altura * 0.15) / 2; setOpacity(obj.mesh, false, 1); 
              ...
              const facingCamera = Math.abs(normal.dot(camDir)) > 0.5;
              const toWall = new THREE.Vector3().subVectors(obj.mesh.position, orbitAlvo);
              toWall.y = 0;
              const isFront = toWall.dot(camDir) < 0;

              if (isFront && facingCamera) {
                  obj.mesh.scale.y = 0.15; obj.mesh.position.y = (obj.nivel * obj.altura) + (obj.altura * 0.15) / 2;
              } else {
