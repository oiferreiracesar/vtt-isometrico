// js/construtor.js - Múltiplos Andares, Gizmo e QoL (Pintar Tudo e Apagar Tudo corrigidos)
...
// =========================================================================
// MOTOR DE CLIQUES E QUALITY OF LIFE (CORREÇÃO DE MAC E BOTÃO DIREITO)
// =========================================================================
canvas?.addEventListener('pointerdown', e => {
  // Permite Botão Esquerdo (0) e Botão Direito (2)
  ...
  // O Botão Direito agora atua nativamente como "Remoção" sem precisar segurar tecla!
  // O metaKey garante o funcionamento no Mac (Cmd)
  ...
  if (isRemocao && modoAtivo !== 'pintura') {
  ...
  if (['parede', 'cerca', 'escada', 'escada_baixo', 'retangulo', 'triangulo', 'octogono', 'telhado'].includes(modoAtivo)) {
    if (e.button === 2) return; // Não construir se for clique direito
    const hit = raycastPlanoBase(e.clientX, e.clientY);
  ...
  // LÓGICA DE PINTURA BLINDADA (Preenchimento Completo e Remoção)
  if (modoAtivo === 'pintura') {
      ...
      // FUNÇÃO: PINTAR TUDO OU APAGAR TUDO
      if (e.shiftKey) {
          ...
          // BLINDAGEM: Verifica se a sala realmente está fechada
          const { celulas, fechada } = encontrarAreaFechada(startX, startZ);
          
          if (!fechada && !isRemocao) {
              showAviso("⚠️ O cômodo está aberto! Feche as paredes para usar o Preenchimento (Shift).");
              return;
          }
...
function aplicarMaterialNaFace(mesh, faceIndex, item) { 
    ...
    if (Array.isArray(mesh.material)) {
        if (mesh.material.length === groupsLen) { novosMateriais = [...mesh.material]; } 
        else { for (let i = 0; i < groupsLen; i++) novosMateriais.push(mesh.material[i] ? mesh.material[i].clone() : mesh.material[0].clone()); }
    } else { for (let i = 0; i < groupsLen; i++) novosMateriais.push(mesh.material.clone()); }
...
canvas?.addEventListener('pointermove', e => {
  ...
  if (e.buttons === 1 && (e.ctrlKey || e.metaKey) && modoAtivo !== 'pintura') {
      const hit = raycastObjetosDoNivel(e.clientX, e.clientY);
      if (hit) executarMarreta(hit.object);
      return;
  }
  ...
  if (hit) {
    ...
    // Mostra o cursor de "Borracha" se estiver usando o Botão Direito, Ctrl ou Cmd
...
// ... Ocultando a continuação idêntica das funções finais para economizar leitura
...
// AQUI ESTAVA O VAZAMENTO DA SALA. AGORA ELE DETECTA SE ESTÁ FECHADA.
